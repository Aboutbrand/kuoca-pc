jQuery(document).ready(function() {
	...
	jQuery( ".layerDiscountPeriod .content p + p" ).each(function( index ) {
		...
		if (today < start_time)	{
			...
			// 타임세일 중
			TimerStart(jQuery(this).parents('.box').find( ".saletime" ),1,end_time);
			jQuery('.layerDiscountPeriod').parents('.box').find( ".timesale_box" ).css("display", "block");
			jQuery(this).parents('.box').find('.sale_per').append(product_sale);
		} else {
